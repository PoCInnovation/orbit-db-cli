...
export default class CounterDrop extends Command {
  static description = 'Delete a counter type database locally (This doesn\'t remove data on other nodes that have the removed database replicated.)'

  static examples = [
    '<%= config.bin %> <%= command.id %> --name=myCounterDbLOL',
    '<%= config.bin %> <%= command.id %> --name=myCounterDbLOL -y',
  ]
  ...
  public async run(): Promise<void> {
    const {flags} = await this.parse(CounterDrop)
