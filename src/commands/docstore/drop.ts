...
export default class DocStoreDrop extends Command {
  public static enableJsonFlag = true
  static description =
    "Delete a docstore type database locally (This doesn't remove data on other nodes that have the removed database replicated.)";

  static examples = [
    "<%= config.bin %> <%= command.id %> --name=myDocstoreDbLOL",
    "<%= config.bin %> <%= command.id %> --name=myDocstoreDbLOL -y",
  ];
  ...
  public async run(): Promise<{ name: string; dropped: boolean }> {
    const { flags } = await this.parse(DocStoreDrop);
    const orbitdb = await startOrbitDB(true, !flags.json);
    const name = await resolveDBIdByName(orbitdb, flags.name, "docstore");
    ...
    const db = await openDB(orbitdb, name, "docstore", { showSpinner: !flags.json });
